...
class OAuthAccount(Base):
    ...
    # Provider info
    provider : Mapped[str] = mapped_column(String(50), nullable=False)  # 'google', '42', 'github', etc.
    provider_user_id : Mapped[str] = mapped_column(String(255), nullable=False)  # User ID from provider
    
    # OAuth data
    email :  Mapped[str | None]  = mapped_column(String(255), nullable=True)  # Email from provider
    profile_data : Mapped[str] = mapped_column(Text, nullable=True)  # JSON string of full profile
